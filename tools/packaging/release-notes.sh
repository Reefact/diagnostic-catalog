...
# belong to that train — so a foundation release never lists a catalog's rule updates,
# and a catalog release never announces a foundation change.
#
# The partition is by Conventional Commit scope (enforced by tools/commit-lint), and the
# scope-to-train mapping comes from tools/trains.sh, shared with the packaging script so
# the two can never disagree. Commits with no scope (bare `ci:`, `build:`, `chore:` ...)
# are infrastructure and are left out of every train: these notes describe what changed
# for the consumer of the package, nothing else.
#
...
while IFS='	' read -r hash subject; do
  [ -z "${subject:-}" ] && continue
  # Extract the first parenthesised scope group ("type(cli,core)!: ..." -> "cli,core");
