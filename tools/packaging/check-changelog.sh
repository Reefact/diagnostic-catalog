...
#
# ── Why this exists ───────────────────────────────────────────────────────────
# Fourteen catalogues were published at 1.0.0 with nothing in their changelogs saying
# so: ten still filed their entries under "Unreleased", and four dated a preview that
# was never cut. Every one of those releases was green. Nothing in the pipeline reads
# a changelog, so nothing could have said otherwise — the omission was caught because
# somebody looked, which is not a check.
#
...
# there. A catalogue keeps its changelog beside its project; the lib train's projects
# carry none and the repository root's CHANGELOG.md is theirs, which is the fallback
# below rather than a special case named by train id — a second train sharing that
# shape would otherwise have to be remembered.
#
# ── Where it runs ─────────────────────────────────────────────────────────────
