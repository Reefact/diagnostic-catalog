...
ADR-0035 is the record in force, and it decides both halves. Three letters at most, abbreviating
the rule prefix when the prefix is longer — the cap it carries forward from ADR-0033, which it
supersedes. The prefix itself is not written down here: it is what the catalogue's own generated
source calls its rules, and a second copy of it would be one more thing to keep true.

When that prefix is already worn by another row, the badge names the subject of the package the
catalogue mirrors instead, and the prefix stays with the catalogue already publishing it. Three
catalogues mirror RS rules: Roslyn holds RS, and PublicApi and BannedApi read API and BAN. Those
two are the only rows here that are neither a prefix nor an abbreviation of one, and nothing below
checks that — the badge a row declares is verified by review.
