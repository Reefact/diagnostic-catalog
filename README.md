...
These twelve are **generated**, never hand-written, and carry ids, categories, help links and
...
| **`DiagnosticCatalog.Cli`**, the `dcat` tool | The generator, as a .NET tool. Point it at an analyzer package or at assemblies on disk and it writes a catalogue the same way this repository writes the thirteen above. |
...
`.Trimming`, `.AspNetCore`, `.Syslib`, `.Roslyn` and `.PublicApi` among the catalogues, and `.Analyzers`, `.Self` and `.Cli`
...
is what unblocked the twelve vendor catalogues, which now ride their own trains.
