...
These eleven are **generated**, never hand-written, and carry ids, categories, help links and
...
| **`DiagnosticCatalog.Cli`**, the `dcat` tool | The generator, as a .NET tool. Point it at an analyzer package or at assemblies on disk and it writes a catalogue the same way this repository writes the twelve above. |
...
`.Trimming`, `.AspNetCore`, `.Syslib` and `.Roslyn` among the catalogues, and `.Analyzers`, `.Self` and `.Cli`
...
is what unblocked the eleven vendor catalogues, which now ride their own trains.
