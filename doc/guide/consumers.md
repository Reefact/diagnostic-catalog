...
```csharp
using Unused = DiagnosticCatalog.Sonar.SonarRules.S1144;
...
```csharp
using static DiagnosticCatalog.Sonar.SonarRules.S1144;
...
[the diagnostics guide](diagnostics.md); here is what each one means in practice.
...
`"S9999"` beside `SonarRules.S1144.Category` — you get the diagnostic and **no** fix, because
...
* [The diagnostics reference](diagnostics.md) — every `DCAT` id, what triggers it, how to configure
  it.
* [Publishing your own catalogue](catalogue-authors.md) — if your team owns an analyzer.
