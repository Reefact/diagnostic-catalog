...
Ces treize-là sont **générés**, jamais écrits à la main, et portent les identifiants, les
...
| **`DiagnosticCatalog.Cli`**, l'outil `dcat` | Le générateur, en outil .NET. Pointez-le vers un paquet d'analyseurs ou vers des assemblages sur disque et il écrit un catalogue comme ce dépôt écrit les quatorze ci-dessus. |
...
`.Trimming`, `.AspNetCore`, `.Syslib`, `.Roslyn`, `.PublicApi` et `.BannedApi` côté catalogues, `.Analyzers`, `.Self` et `.Cli` côté outils.
...
release qui a débloqué les treize catalogues d'éditeurs, qui roulent désormais sur leurs propres
trains.
