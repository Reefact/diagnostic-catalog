# ADR-{number} | {Short Title}

<!-- An ADR records a DECISION and its REASONING, not how it is implemented.
     Keep code, configuration, YAML, exact flags, XML/command snippets and
     step-by-step "how it is built" OUT of every section — that is
     specification: it lives in the code and beside it, which this ADR links
     to. Test: if the implementation changed but the decision stood, this ADR
     should not need editing. See doc/adr/README.md. -->

**Status:** Proposed
**Proposed:** YYYY-MM-DD
**Decision Makers:** {Names or team}

<!-- Accepting this ADR ADDS an `**Accepted:** YYYY-MM-DD` line below `Proposed:`
     and leaves `Proposed:` untouched; no date is ever overwritten. A supersession
     adds no date at all. See README.md, "Title and header". -->

## Context

<!-- Facts only — no justification of the chosen solution. Everything the
     Rationale argues from must be stated here first: business context,
     requirements, technical/architectural/operational constraints, security,
     performance, cost, team skills, existing limitations, organizational
     constraints, external dependencies, deadlines, known risks. Someone
     unfamiliar with the project must understand why a decision had to be
     made. -->

## Decision

<!-- One single sentence. No justification, no alternatives, no history, no
     implementation details unless they are part of the decision itself. -->

## Rationale

<!-- Argument ONLY — why this decision is the best choice given the Context.
     Every argument must trace back to a fact in Context; if one is missing,
     add it to Context first. Cover: why the decision satisfies the
     requirements, which constraints it addresses, which trade-offs were
     accepted, why the benefits outweigh the drawbacks.
     Do NOT put implementation detail here — no code, config, YAML, exact
     flags, XML/command snippets, no guard-by-guard or step-by-step "how it is
     built". That is specification: link to the code instead. Naming a
     mechanism's role and why it exists is argument (keep it); documenting how
     it is wired is specification (link out). -->

## Alternatives Considered

### {Alternative 1}

<!-- Why it was considered. -->

<!-- Why it was ultimately rejected. -->

### {Alternative 2}

...

## Consequences

### Positive

* ...

### Negative

* ...

### Risks

* ...

## Follow-up Actions

* ...

## References

<!-- Optional: related ADRs, RFCs, specifications, benchmarks, design
     documents, pull requests, issues, diagrams. -->

* ...
