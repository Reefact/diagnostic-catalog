...
* [ADR-0004](0004-state-the-coding-rules-where-an-agent-can-act-on-them.md) — the same argument for
  the coding rules: a rule stated where no compiler and no agent can read it is enforced by nobody.
* [ADR-0013](0013-write-the-shell-tooling-for-posix-sh-not-bash.md) — why the two new scripts are
  POSIX `sh`.
